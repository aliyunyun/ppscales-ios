...
@property (nonatomic, strong) void(^myBMDJStartHandler)(BOOL isSuccess);

@property (nonatomic, strong) void(^myBMDJExitHandler)(BOOL isSuccess);

@property (nonatomic, strong) void(^myBMDJTimeIntervalHandler)(BOOL isEnd, NSInteger timeInterval, BOOL isFailed);

@property (nonatomic, assign) BOOL isConnectBMDJ;
@end
...
- (void)centralManager:(CBCentralManager *)central didDiscoverPeripheral:(CBPeripheral *)peripheral advertisementData:(NSDictionary<NSString *,id> *)advertisementData RSSI:(NSNumber *)RSSI{
    ...
    if (advData.length != 19 && advData.length != 17 && advData.length != 8 && advData.length != 25) {
        return;
    }
    
    if (!self.isConnectBMDJ) {
